...
import { NAV_ITEMS, canAccess, type MenuItem } from "../config/menu";
...
const AppSidebar: React.FC = () => {
  ...
  const navItems = useMemo<MenuItem[]>(() => {
    return NAV_ITEMS.filter((item) => {
      if (!canAccess(user?.role, item.roles)) return false;
      if (!item.subItems) return true;
      return item.subItems.some((sub) => canAccess(user?.role, sub.roles));
    });
  }, [user?.role]);
