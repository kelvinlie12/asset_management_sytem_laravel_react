...
class AssetDamageController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $damages = AssetDamage::query()
            ->with(['asset.room', 'asset.category', 'user', 'responsible', 'performedBy'])
    ...
    public function store(StoreAssetDamageRequest $request): JsonResponse
    {
        ...
        $damage = DB::transaction(function () use ($validated, $asset, $photoPath) {
        ...
        return ApiResponse::created([
            'damage' => new AssetDamageResource(
                $damage->load(['asset.room', 'asset.category', 'user', 'responsible', 'performedBy'])
            ),
    ...
    public function show(AssetDamage $assetDamage): JsonResponse
    {
        return ApiResponse::success([
            'damage' => new AssetDamageResource(
                $assetDamage->load(['asset.room', 'asset.category', 'user', 'responsible', 'performedBy'])
            ),
        ]);
    }
}
